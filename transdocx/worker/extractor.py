...
from transdocx.document.document import TextSegment, TableCellSegment, ChartSegment, SmartArtSegment, RunInfo
from transdocx.utils.is_numeric import is_numeric
from transdocx.utils.decorator import progress_tracker, timer, log_errors
import logging
...
class Extractor:
    ...
    def __init__(self, input_file: str, checkpoint_file: str):
        ...
        #thêm từ đây
        # ĐẢM BẢO THƯ MỤC CHỨA CHECKPOINT TỒN TẠI
        checkpoint_dir = os.path.dirname(checkpoint_file)
        if checkpoint_dir and not os.path.exists(checkpoint_dir):
            os.makedirs(checkpoint_dir, exist_ok=True)
            self.logger.info(f"Created directory for checkpoint: {checkpoint_dir}")
        #đến đây
